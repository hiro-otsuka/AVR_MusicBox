...
### 録音形式の変換
前述の spwave 等を用いて、PCMモノラル/8bit/8KHz サンプリングの WAVファイルに変換する。なお、当方の環境では、spwave で保存時に形式 "Microsoft PCM(libsndfile)" ではエラーとなった。"Microsoft PCM" を選択すれば正常に保存できた。ADPCMにする場合は、"WAVE IMA ADPCM(libsndfile)"を選択する。
